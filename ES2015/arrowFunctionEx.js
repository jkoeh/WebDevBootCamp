/* 1 - Refactor the following code to 
use ES2015 arrow functions - make sure your function 
...
*/
let tripleAndFilter = arr => arr.map(value => value * 3)
...
*/
let doubleOddNumbers = arr => arr.filter(val => val % 2 !== 0)
    .map(val => val * 2);
...
let mapFilterAndReduce = (arr) => arr.map((val) => val.firstName)
    ...
    .reduce((acc, next) => { acc[next] = next.length; return acc }, {});



/* 4 - Write a function called createStudentObj which accepts two parameters, firstName and lastName and returns an object with the
keys of firstName and lastName with the values as the parameters passed 
to the function.
